              ...
              <div class="card-body">
                <table class="table">
                  <thead>
                    ...
                    <th class="py-0 px-1">Tipo Ingreso</th>
                  ...
                  <tbody>
                    <tr v-for="(m, i) in datosPaginados">
                      ...
                      <td data-label="Tipo Ingreso" class="py-0 px-1">{{ m.t_tipo_ingreso.nombre }}</td>
                      <td data-label="Monto" class="py-0 px-1">{{ m.monto | format_number}}</td>
                      <td data-label="Acciones" class="py-0 px-1">
                        <div data-label="Acciones" class="btn-group">
              ...
              <div class="modal-content">
                ...
                <div class="clas-12">
                  <div class="form-group">
                    ...
                    <select name="" id="" class="form-control" v-model="t_tipo_ingreso_id"">
                      <option v-for="m in t_t_ingresos" :value="m.id">{{ m.nombre }}</option>
                    </select>
                  </div>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      nombre: '',
      apiUrl: 'tGIngresos',
      apiUrlFiltro: 'tGIngreso/filtro?filtro=',
 ...
 return parseInt(value).toLocaleString("de-DE")
    ...
    listFilter() {
      ...
      if (buscar != '') {
        return this.list.filter((a) => {
          ...
          let ingreso = a.t_tipo_ingreso.nombre != null ? a.t_tipo_ingreso.nombre : ''
          return nombre.toLowerCase().indexOf(buscar.toLowerCase()) != -1 || periodo.toLowerCase().indexOf(buscar.toLowerCase()) != -1 || ingreso.toLowerCase().indexOf(buscar.toLowerCase()) != -1
        })
      }
      return this.list
    },
  ...
  methods: {
    ...
    async Save() {
      ...
      try {
        const operacion = {
          t_tipo_ingreso_id: this.t_tipo_ingreso_id,
    ...
    abrirEditar(m){
      ...
      this.t_tipo_ingreso_id = m.t_tipo_ingreso_id,
      this.monto = m.monto
        },
        cleanModal(){
      this.id = ''
  ...
  mounted() {
    ...
    this.$nextTick(async () => {
      try {
        await Promise.all([this.GET_DATA(this.apiUrl), this.GET_DATA('tPeriodos'), this.GET_DATA('tTipoIngresos'), this.GET_DATA('empresas')]).then((v) => {
          this.list = v[0]
          this.periodos = v[1]
          this.t_t_ingresos = v[2]
          this.empresas = v[3]
        })
      } catch (e) {
        console.log(e);
