                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="(m, i) in datosPaginados">
                      ...
                      <td data-label="Proceso" class="py-0 px-1">{{ m.t_tarea.nombre }}</td>
                      <td data-label="Estado" class="py-0 px-1">{{ m.t_p_remuneracione.nombre }}</td>
              ...
              <div class="modal-content">
                ...
                <div class="clas-12">
                  <div class="form-group">
                    ...
                    <select name="" id="" class="form-control" v-model="t_tarea_id">
                      <option v-for="m in tareas" :value="m.id">{{ m.nombre }}</option>
                    </select>
                  </div>
                </div>
                <div class="clas-12">
              ...
              <div class="modal-content">
                ...
                <div class="clas-12">
                  <div class="form-group">
                    ...
                    <select name="" id="" class="form-control" v-model="t_p_remuneracione_id">
                      <option v-for="m in t_p_remuneraciones  " :value="m.id">{{ m.nombre }}</option>
                    </select>
                  </div>
                </div>
                <div class="modal-footer">
...
export default {
  ...
  data() {
    return {
      ...
      buscar:"",
      nombre: '',
      apiUrl: 'tGRemuneraciones',
      apiUrlFiltro: 'tGRemuneracione/filtro?filtro=',
      page: 'Gestion del Contador',
      modulo: 'Proceso Remuneraciones',
 ...
 return parseInt(value).toLocaleString("de-DE")
    ...
    listFilter() {
      ...
      if (buscar != '') {
        return this.list.filter((a) => {
          ...
          let estado = a.t_p_remuneracione.nombre != null ? a.t_p_remuneracione.nombre : ''
          return nombre.toLowerCase().indexOf(buscar.toLowerCase()) != -1 || periodo.toLowerCase().indexOf(buscar.toLowerCase()) != -1 || estado.toLowerCase().indexOf(buscar.toLowerCase()) != -1
        })
  ...
  methods: {
    ...
    async Save() {
      ...
      try {
        const operacion = {
          t_p_remuneracione_id: 1,
          t_tarea_id: this.t_tarea_id,
    ...
    async Update() {
      {
            ...
            try {
                const res = await this.$api.$put(this.apiUrl + "/" + this.id,{
                    ...
                    't_tarea_id':this.t_tarea_id,
                    't_periodo_id':this.t_periodo_id,
                    't_p_remuneracione_id':this.t_p_remuneracione_id,
                    'id':this.id,
                });
                console.log(res)
                this.$swal.fire({
    ...
    abrirEditar(m){
      ...
      this.t_tarea_id = m.t_tarea_id
      this.t_periodo_id = m.t_periodo_id
      this.t_p_remuneracione_id = m.t_p_remuneracione_id
        },
        cleanModal(){
      this.id = ''
      this.empresa_id = ''
  ...
  mounted() {
    ...
    this.$nextTick(async () => {
      try {
        await Promise.all([this.GET_DATA(this.apiUrlFiltro + this.user.empresa_id), this.GET_DATA('tPeriodos'), this.GET_DATA('tPRemuneraciones'), this.GET_DATA('empresas'), this.GET_DATA('tTareas')]).then((v) => {
          this.list = v[0]
          this.periodos = v[1]
          this.t_p_remuneraciones = v[2]
          this.empresas = v[3]
          this.tareas = v[4]
        })
      } catch (e) {
